...
function setupGlobalEventListeners() {
    ...
    document.addEventListener('click', function(event) {
        ...
        // Se clicou no trigger, não fechar
...
window.limparDadosDuplicadosGestao = limparDadosDuplicadosGestao;
    ...
    // Inicializar sistema completo do menu do usuário
    setTimeout(() => {
        initializeUserMenu();
    }, 100);
...
// ========== SISTEMA COMPLETO DO MENU DO USUÁRIO ==========

// Função para inicializar todo o sistema do menu do usuário
function initializeUserMenu() {
    console.log('👤 Inicializando sistema completo do menu do usuário...');
    ...
    if (!userMenuTrigger) {
        console.error('❌ Elemento .user-menu-trigger não encontrado');
        return;
    }
    
    if (!userMenuDropdown) {
        console.error('❌ Elemento #userMenuDropdown não encontrado');
        return;
    }
    
    // Marcar como inicializado para evitar duplicações
    userMenuTrigger.setAttribute('data-initialized', 'true');
    
    // Configurar evento de clique no trigger
    userMenuTrigger.addEventListener('click', function(e) {
        console.log('🖱️ Clique no trigger detectado!');
        e.preventDefault();
        e.stopPropagation();
        toggleUserMenu();
    });
    ...
    // Configurar eventos das opções do menu
    setupUserMenuOptions();
    
    console.log('✅ Sistema do menu do usuário inicializado com sucesso');
}
...
function loadUserInfo() {
    ...
    // Obter informações do usuário (pode vir do localStorage, sessionStorage ou API)
...
function getUserInfo() {
    // Primeiro tenta obter do localStorage/sessionStorage
    ...
    if (!userInfo.name) {
        userInfo = {
            ...
            role: 'Administrador',
            avatar: 'https://ui-avatars.com/api/?name=A&background=667eea&color=fff&size=128',
            initials: 'A'
        };
...
// Função para atualizar a exibição do usuário no DOM
function updateUserDisplay(userInfo) {
    console.log('🖼️ Atualizando exibição do usuário:', userInfo);
    
    // Atualizar nome do usuário
    const userNameElements = document.querySelectorAll('#userName, #userNameLarge');
    ...
    // Atualizar avatars
    const avatarElements = document.querySelectorAll('#userAvatar, #userAvatarLarge');
    avatarElements.forEach(el => {
        if (el) {
            el.src = userInfo.avatar;
            el.alt = `Avatar de ${userInfo.name}`;
        }
    });
}

// Função para configurar eventos das opções do menu
function setupUserMenuOptions() {
    console.log('⚙️ Configurando eventos das opções do menu...');
...
function logout() {
    ...
    showConfirm(
        ...
        function() {
            ...
            // Mostrar loading
            showInfo('Saindo...', 'Redirecionando para a página de login');
        ...
        function() {
            ...
            toggleUserMenu(); // Fechar o menu se cancelou
        }
    );
}
...
function toggleUserMenu() {
    console.log('🔄 Alternando menu do usuário...');
    const userMenu = document.querySelector('.user-menu');
    const dropdown = document.getElementById('userMenuDropdown');
    ...
    if (userMenu.classList.contains('active')) {
        ...
        // Fechar outros menus se existirem
...
function showUserProfile() {
    ...
    toggleUserMenu(); // Fechar o menu primeiro
        ...
        <div class="modal-content" style="max-width: 500px;">
            ...
            <div class="profile-content" style="padding: 20px;">
                <div class="profile-avatar-section" style="text-align: center; margin-bottom: 20px;">
                    <div class="profile-avatar-large" style="display: inline-block; position: relative;">
                        <img src="${userInfo.avatar}" alt="Avatar" style="width: 100px; height: 100px; border-radius: 50%; border: 4px solid #667eea;">
                        <button class="change-avatar-btn" onclick="changeUserAvatar()" style="position: absolute; bottom: 0; right: 0; background: #667eea; color: white; border: none; border-radius: 50%; width: 30px; height: 30px; cursor: pointer;">
                ...
                <form id="profileForm" class="profile-form">
                    <div class="form-group">
                        ...
                        <input type="text" id="profileName" value="${userInfo.name}" class="form-control">
                    </div>
                    
                    <div class="form-group">
                        <label for="profileEmail">Email:</label>
                        <input type="email" id="profileEmail" value="${userInfo.email}" class="form-control">
                    </div>
                    
                    <div class="form-group">
                        <label for="profileRole">Função:</label>
                        <select id="profileRole" class="form-control">
                            <option value="Administrador" ${userInfo.role === 'Administrador' ? 'selected' : ''}>Administrador</option>
                            <option value="Usuário" ${userInfo.role === 'Usuário' ? 'selected' : ''}>Usuário</option>
                            <option value="Supervisor" ${userInfo.role === 'Supervisor' ? 'selected' : ''}>Supervisor</option>
                        </select>
                    </div>
                    
                    <div class="form-group">
                        <label for="profileTheme">Tema do Sistema:</label>
            ...
            <div class="modal-footer">
                <button type="button" class="btn-primary" onclick="saveUserProfile()">
                    <i class="fas fa-save"></i> Salvar Alterações
                </button>
                <button type="button" class="btn-secondary" onclick="closeUserProfileModal()">
...
// Funções auxiliares para o modal de perfil
...
function saveUserProfile() {
    ...
    const userInfo = {
        ...
        role: role,
        theme: theme,
        avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=667eea&color=fff&size=128`,
...
function changeUserAvatar() {
    console.log('📸 Alterando avatar do usuário...');
    ...
    input.onchange = function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                const avatarImg = document.querySelector('#userProfileModal .profile-avatar-large img');
                if (avatarImg) {
                    avatarImg.src = e.target.result;
                }
                
                showSuccess('Avatar Atualizado', 'Sua foto de perfil foi alterada!');
            };
...
function showUserSettings() {
    ...
    // Aqui você pode implementar a lógica para mostrar as configurações
    showNotification('Configurações', 'Funcionalidade em desenvolvimento', 'info');
    toggleUserMenu(); // Fecha o menu
}

// Função para carregar informações do usuário
function loadUserInfo() {
    const userName = sessionStorage.getItem('mdu_user') || 'Usuário';
    const userEmail = sessionStorage.getItem('mdu_user_email') || 'usuario@inovar.com';
    ...
    // Carregar avatar baseado no nome do usuário
    const avatarElements = document.querySelectorAll('#userAvatar, #userAvatarLarge');
    avatarElements.forEach(element => {
        // Gerar avatar baseado nas iniciais do usuário
        const initials = userName.split(' ').map(name => name.charAt(0)).join('').toUpperCase();
        element.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(initials)}&background=667eea&color=fff&size=128`;
    });
}
...
// Função para limpar dados de exemplo do localStorage
function clearExampleData() {
    console.log('🧹 Limpando dados de exemplo do localStorage...');
    
    // Limpar dados de gestão se contiver dados de exemplo
    const savedGestaoData = localStorage.getItem('gestaoData');
    if (savedGestaoData) {
        try {
            const gestaoData = JSON.parse(savedGestaoData);
            
            // Verificar se contém dados de exemplo
            const hasExampleData = gestaoData.projetos?.some(p => 
                p.nome === 'PROJETO F' || p.nome === 'MDU-TOA' || p.nome === 'Projeto MDU'
            ) || gestaoData.supervisores?.some(s => 
                s.nome === 'JESSICA' || s.nome === 'ROBERTO' || s.nome === 'VALNEI' || s.nome === 'CARLOS'
            ) || gestaoData.cidades?.some(c => 
                c.nome === 'São Paulo' || c.nome === 'Rio de Janeiro'
            ) || gestaoData.tiposAcao?.some(t => 
                t.nome === 'ATIVAÇÃO' || t.nome === 'CONSTRUÇÃO' || t.nome === 'VISTORIA'
            );
            
            if (hasExampleData) {
                console.log('🗑️ Removendo dados de exemplo do localStorage...');
                localStorage.removeItem('gestaoData');
                console.log('✅ Dados de exemplo removidos');
            }
        } catch (error) {
            console.error('❌ Erro ao verificar dados de gestão:', error);
        }
    }
    
    // Limpar dados de endereços se contiver dados de exemplo
    const savedEnderecosData = localStorage.getItem('enderecosData');
    if (savedEnderecosData) {
        try {
            const enderecosData = JSON.parse(savedEnderecosData);
            
            // Verificar se contém dados de exemplo
            const hasExampleData = enderecosData.some(e => 
                e.projeto === 'PROJETO F' || e.projeto === 'MDU-TOA' || e.projeto === 'Projeto MDU' ||
                e.supervisor === 'JESSICA' || e.supervisor === 'ROBERTO' ||
                e.cidade === 'São Paulo' || e.cidade === 'Rio de Janeiro'
            );
            
            if (hasExampleData) {
                console.log('🗑️ Removendo dados de exemplo de endereços do localStorage...');
                localStorage.removeItem('enderecosData');
                console.log('✅ Dados de exemplo de endereços removidos');
            }
        } catch (error) {
            console.error('❌ Erro ao verificar dados de endereços:', error);
        }
    }
}
...
function applyDataCorrection() {
    ...
    updateSubProjetosChart();
    updateCidadesChart();
    updateHpProjetosChart();
    updateSupervisorStatusChart();
    renderEquipeRanking();
    renderEquipeStatusRanking();
...
function loadProjectsTable() {
    ...
    const projetosLimitados = projetosArray.slice(0, 50);
...
function loadSubProjectsTable() {
    ...
    console.log('📊 Sub projetos encontrados:', subProjetosArray.length);
    
    if (subProjetosArray.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align: center;">Nenhum sub projeto encontrado</td></tr>';
        return;
    }
    
    subProjetosArray.forEach((subprojeto, index) => {
        const row = document.createElement('tr');
...
function corrigirEstruturaGestaoData() {
    ...
    if (gestaoData.tiposAcao) {
        ...
        // Não criar tipos de ação padrão - deixar vazio
    ...
    // Não criar projetos padrão - deixar vazio se não há dados
    // if (dadosCorrigidos.projetos.length === 0) {
    //     // Removido: não criar dados de exemplo automaticamente
    // }
...
function reorganizarGestaoData() {
    ...
    // Salvar dados reorganizados (limpos)
    localStorage.setItem('gestaoData', JSON.stringify(gestaoData));
...
// Função para limpar completamente os dados de gestão
function limparDadosGestao() {
    console.log('🧹 Limpando dados de gestão...');
    ...
    console.log('✅ Dados de gestão limpos com sucesso!');
    showSuccess('Dados Limpos!', 'Todas as tabelas de gestão foram limpas com sucesso!');
}

// Tornar função global
window.limparDadosGestao = limparDadosGestao;
...
function deleteProject(id, source) {
    ...
    showConfirm(
        ...
        () => {
            try {
                if (source === 'manual') {
                    const index = gestaoData.projetos.findIndex(p => p.id == id);
                    if (index !== -1) {
                        gestaoData.projetos.splice(index, 1);
                        localStorage.setItem('gestaoData', JSON.stringify(gestaoData));
                        loadProjectsTable();
    ...
    showConfirm(
        ...
        () => {
            try {
                ...
                const index = gestaoData.subprojetos.findIndex(sp => sp.id == id);
                if (index !== -1) {
                    gestaoData.subprojetos.splice(index, 1);
                    localStorage.setItem('gestaoData', JSON.stringify(gestaoData));
                    loadSubProjectsTable();
...
function criarFormularioDinamico() {
    ...
    const camposFormulario = headers.map(header => {
        ...
        if (header.toUpperCase().includes('PROJETO') && !header.toUpperCase().includes('SUB')) {
            ...
            tipoCampo = 'select';
            opcoes = gestaoData.tiposAcao ? gestaoData.tiposAcao.map(ta => ta.nome) : [];
            console.log(`  → Tipo: ${tipoCampo}, Opções: ${opcoes.length} (${opcoes.join(', ')})`);
...
function updateInfraStats() {
    ...
    try {
        ...
        console.log('🔍 Elementos dos cards de infraestrutura encontrados:', {
            total: !!totalElement,
        ...
        if (totalElement) {
            ...
            console.log('✅ Total de infraestrutura atualizado:', stats.totalRegistros);
        } else {
            console.warn('⚠️ Elemento total de infraestrutura não encontrado');
        }
        
        if (enderecosElement) {
            enderecosElement.textContent = stats.enderecosDistintos;
            console.log('✅ Endereços de infraestrutura atualizados:', stats.enderecosDistintos);
        } else {
            console.warn('⚠️ Elemento endereços de infraestrutura não encontrado');
        }
        
        if (equipesElement) {
            equipesElement.textContent = stats.equipesDistintas;
            console.log('✅ Equipes de infraestrutura atualizadas:', stats.equipesDistintas);
        } else {
            console.warn('⚠️ Elemento equipes de infraestrutura não encontrado');
        }
        
        if (produtividadeElement) {
            produtividadeElement.textContent = `${stats.produtividade}%`;
            console.log('✅ Produtividade de infraestrutura atualizada:', `${stats.produtividade}%`);
        } else {
            console.warn('⚠️ Elemento produtividade de infraestrutura não encontrado');
        }
...
function applyInfraFilters() {
    ...
    try {
        ...
        const projeto = document.getElementById('infraFilterProjeto').value;
        const subProjeto = document.getElementById('infraFilterSubProjeto').value;
        const equipe = document.getElementById('infraFilterEquipe').value;
        const status = document.getElementById('infraFilterStatus').value;
        const cidade = document.getElementById('infraFilterCidade').value;
        const supervisor = document.getElementById('infraFilterSupervisor').value;
        const tipoAcao = document.getElementById('infraFilterTipoAcao').value;
        
        console.log('🔍 Filtros aplicados:', { projeto, subProjeto, equipe, status, cidade, supervisor, tipoAcao });
        ...
        let filteredDinamicos = dynamicTableData.data.filter(row => {
            const projetoMatch = !projeto || (columnNames.PROJETO && row[columnNames.PROJETO]?.toString().trim() === projeto);
            const subProjetoMatch = !subProjeto || (columnNames['SUB PROJETO'] && row[columnNames['SUB PROJETO']]?.toString().trim() === subProjeto);
            const equipeMatch = !equipe || (columnNames.EQUIPE && row[columnNames.EQUIPE]?.toString().trim() === equipe);
            const statusMatch = !status || (columnNames.STATUS && row[columnNames.STATUS]?.toString().trim() === status);
            const cidadeMatch = !cidade || (columnNames.CIDADE && row[columnNames.CIDADE]?.toString().trim() === cidade);
            const supervisorMatch = !supervisor || (columnNames.SUPERVISOR && row[columnNames.SUPERVISOR]?.toString().trim() === supervisor);
            const tipoAcaoMatch = !tipoAcao || (columnNames['TIPO DE AÇÃO'] && row[columnNames['TIPO DE AÇÃO']]?.toString().trim() === tipoAcao);
            
            return projetoMatch && subProjetoMatch && equipeMatch && statusMatch && cidadeMatch && supervisorMatch && tipoAcaoMatch;
        });
        
        // Usar apenas dados da tabela dinâmica filtrados
        const totalFiltered = filteredDinamicos.length;
        
        console.log(`📊 Dados filtrados: ${totalFiltered} registros encontrados`);
        console.log('🔍 Exemplo de dados filtrados:', filteredDinamicos.slice(0, 3));
        
        // Verificar se há dados para filtrar
        if (!filteredDinamicos || filteredDinamicos.length === 0) {
        ...
        try {
            updateProjetosChartWithData(filteredDinamicos);
            console.log('✅ Gráfico de projetos atualizado');
        } catch (error) {
            console.error('❌ Erro ao atualizar gráfico de projetos:', error);
        }
        
        try {
            updateSubProjetosChartWithData(filteredDinamicos);
            console.log('✅ Gráfico de sub-projetos atualizado');
        } catch (error) {
            console.error('❌ Erro ao atualizar gráfico de sub-projetos:', error);
        }
        
        try {
            updateCidadesChartWithData(filteredDinamicos);
            console.log('✅ Gráfico de cidades atualizado');
        } catch (error) {
            console.error('❌ Erro ao atualizar gráfico de cidades:', error);
        }
        
        try {
            updateHpProjetosChartWithData(filteredDinamicos);
            console.log('✅ Gráfico HP por projeto atualizado');
        } catch (error) {
            console.error('❌ Erro ao atualizar gráfico HP por projeto:', error);
        }
        
        try {
            updateSupervisorStatusChartWithData(filteredDinamicos);
            console.log('✅ Gráfico supervisor/status atualizado');
        } catch (error) {
            console.error('❌ Erro ao atualizar gráfico supervisor/status:', error);
        }
        
        // Atualizar tabelas de ranking com dados filtrados
        console.log('🔄 Atualizando tabelas de ranking com dados filtrados...');
        
        try {
            updateEquipeRankingWithData(filteredDinamicos);
            console.log('✅ Tabela ranking equipes atualizada');
        } catch (error) {
            console.error('❌ Erro ao atualizar tabela ranking equipes:', error);
        }
        
        try {
            updateEquipeStatusRankingWithData(filteredDinamicos);
            console.log('✅ Tabela ranking status atualizada');
        } catch (error) {
            console.error('❌ Erro ao atualizar tabela ranking status:', error);
        }
        
        // Atualizar horário da última atualização
        updateInfraLastUpdateTime();
        
        // Pequeno delay para garantir que todos os gráficos sejam atualizados
        setTimeout(() => {
            console.log(`✅ Filtros aplicados: ${totalFiltered} registros encontrados`);
            console.log('📊 Todos os gráficos e tabelas atualizados com dados filtrados');
            
            // Mostrar notificação de sucesso
            if (typeof showSuccess === 'function') {
                showSuccess('Filtros Aplicados!', `${totalFiltered} registros encontrados. Todos os gráficos e tabelas foram atualizados.`);
            }
            
            // Forçar re-renderização dos gráficos se necessário
            if (window.projetosChart && typeof window.projetosChart.update === 'function') {
                window.projetosChart.update('none');
            }
            if (window.subProjetosChart && typeof window.subProjetosChart.update === 'function') {
                window.subProjetosChart.update('none');
            }
            if (window.cidadesChart && typeof window.cidadesChart.update === 'function') {
                window.cidadesChart.update('none');
            }
            if (window.hpProjetosChart && typeof window.hpProjetosChart.update === 'function') {
                window.hpProjetosChart.update('none');
            }
            if (window.supervisorStatusChart && typeof window.supervisorStatusChart.update === 'function') {
                window.supervisorStatusChart.update('none');
            }
        }, 100);
    } catch (error) {
...
function clearInfraFilters() {
    ...
    try {
        ...
        document.getElementById('infraFilterProjeto').value = '';
        document.getElementById('infraFilterSubProjeto').value = '';
        document.getElementById('infraFilterEquipe').value = '';
        document.getElementById('infraFilterStatus').value = '';
        document.getElementById('infraFilterCidade').value = '';
        document.getElementById('infraFilterSupervisor').value = '';
        document.getElementById('infraFilterTipoAcao').value = '';
        
        // Atualizar estatísticas com todos os dados
        updateInfraStatsWithData([], dynamicTableData.data);
        
        // Atualizar TODOS os gráficos com todos os dados
        console.log('🔄 Atualizando gráficos com dados completos...');
        
        try {
            updateProjetosChartWithData(dynamicTableData.data);
            console.log('✅ Gráfico de projetos atualizado');
        } catch (error) {
            console.error('❌ Erro ao atualizar gráfico de projetos:', error);
        }
        ...
        try {
            // Usar a função de criação original para garantir dados completos
            createSubProjetosCombinedChart();
            console.log('✅ Gráfico de sub-projetos recriado com dados completos');
        } catch (error) {
            console.error('❌ Erro ao recriar gráfico de sub-projetos:', error);
        }
        
        try {
            // Usar a função de criação original para garantir dados completos
            createCidadesCombinedChart();
            console.log('✅ Gráfico de cidades recriado com dados completos');
        } catch (error) {
            console.error('❌ Erro ao recriar gráfico de cidades:', error);
        }
        
        try {
            // Usar a função de criação original para garantir dados completos
            createHpProjetosBarChart();
            console.log('✅ Gráfico HP por projeto recriado com dados completos');
        } catch (error) {
            console.error('❌ Erro ao recriar gráfico HP por projeto:', error);
        }
        
        try {
            // Usar a função de criação original para garantir dados completos
            createSupervisorStatusBarChart();
            console.log('✅ Gráfico supervisor/status recriado com dados completos');
        } catch (error) {
            console.error('❌ Erro ao recriar gráfico supervisor/status:', error);
        }
        
        // Atualizar tabelas de ranking com todos os dados
        console.log('🔄 Atualizando tabelas de ranking com dados completos...');
        
        try {
            // Usar a função de criação original para garantir dados completos
            generateEquipeRanking();
            renderEquipeRanking();
            console.log('✅ Tabela ranking equipes recriada com dados completos');
        } catch (error) {
            console.error('❌ Erro ao recriar tabela ranking equipes:', error);
        }
        
        try {
            // Usar a função de criação original para garantir dados completos
            generateEquipeStatusRanking();
            renderEquipeStatusRanking();
            console.log('✅ Tabela ranking status recriada com dados completos');
        } catch (error) {
            console.error('❌ Erro ao recriar tabela ranking status:', error);
        }
        ...
        // Mostrar notificação de sucesso
        if (typeof showSuccess === 'function') {
            showSuccess('Filtros Limpos!', 'Todos os filtros foram removidos. Gráficos e tabelas atualizados com dados completos.');
        }
        
        console.log('✅ Filtros da infraestrutura limpos');
        console.log('📊 Todos os gráficos e tabelas atualizados com dados completos');
    } catch (error) {
...
function updateSubProjetosChartWithData(subProjetosData) {
    ...
    // Processar dados filtrados ou usar dados completos
    let processedData = [];
    
    if (subProjetosData && Array.isArray(subProjetosData) && subProjetosData.length > 0) {
        console.log('📊 Processando dados filtrados para sub projetos...');
        
        // Obter nomes exatos das colunas
        const columnNames = getDynamicTableColumnNames();
        const subProjetoCol = columnNames['SUB PROJETO'];
        
        if (subProjetoCol) {
            // Agrupar dados filtrados por sub projeto
            const subProjetosCount = {};
            let total = 0;
            
            subProjetosData.forEach(row => {
            ...
            processedData = Object.entries(subProjetosCount).map(([nome, quantidade]) => ({
                nome,
                quantidade,
            ...
            // Ordenar por quantidade (decrescente)
            processedData.sort((a, b) => b.quantidade - a.quantidade);
            
            console.log(`✅ Processados ${processedData.length} sub projetos dos dados filtrados`);
        } else {
            console.warn('⚠️ Coluna SUB PROJETO não encontrada');
        }
    } else {
        console.log('📊 Usando dados completos para sub projetos...');
        processedData = calculateSubProjetosData();
    }
    ...
    const labels = processedData.map(p => p.nome);
    const data = processedData.map(p => p.quantidade);
    const percentages = processedData.map(p => parseFloat(p.percentual));
    ...
    window.subProjetosChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Análise de Sub Projetos - Quantidade e Percentual',
                    font: {
                        size: 16,
                        weight: 'bold'
                    },
...
function updateCidadesChartWithData(cidadesData) {
    ...
    if (cidadesData && Array.isArray(cidadesData) && cidadesData.length > 0) {
        console.log('📊 Processando dados filtrados para cidades...');
        
        // Obter nomes exatos das colunas
        const columnNames = getDynamicTableColumnNames();
        const cidadeCol = columnNames.CIDADE;
        
        if (cidadeCol) {
            // Agrupar dados filtrados por cidade
            const cidadesCount = {};
            let total = 0;
            ...
            // Converter para formato esperado pelo gráfico
            processedData = Object.entries(cidadesCount).map(([nome, quantidade]) => ({
                nome,
                quantidade,
                percentual: total > 0 ? ((quantidade / total) * 100).toFixed(1) : 0
            }));
            ...
            console.log(`✅ Processadas ${processedData.length} cidades dos dados filtrados`);
        } else {
            console.warn('⚠️ Coluna CIDADE não encontrada');
        }
    } else {
        console.log('📊 Usando dados completos para cidades...');
        processedData = calculateCidadesData();
    }
    ...
    window.cidadesChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Análise de Cidades - Quantidade e Percentual',
                    font: {
                        size: 16,
                        weight: 'bold'
                    },
...
function updateHpProjetosChartWithData(data) {
    console.log('🔄 Atualizando gráfico HP por projeto com dados filtrados...');
    ...
    if (data && Array.isArray(data)) {
        // Obter nomes exatos das colunas da tabela dinâmica
        const columnNames = getDynamicTableColumnNames();
        const projetoCol = columnNames.PROJETO;
        const hpCol = columnNames.HP || 'HP'; // HP pode não estar na função getDynamicTableColumnNames
        
        console.log('🔍 Colunas para HP por projeto:', { projetoCol, hpCol });
    ...
    // Se não há dados suficientes, não criar gráfico
    ...
    const chartData = labels.map(label => projetosHp[label]);
    ...
    console.log('✅ Gráfico HP por projeto atualizado com dados filtrados');
}
    ...
    window.supervisorStatusChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Endereços por Supervisor e Status',
                    font: {
                        size: 16,
                        weight: 'bold'
                    },
...
function createProjetosCombinedChart() {
    ...
    // Se não há dados suficientes, mostrar mensagem de "Sem dados"
    if (total === 0 || Object.keys(projetosCount).length === 0) {
        console.log('📊 Nenhum dado disponível para exibir...');
        // Não criar gráfico se não há dados
        return;
    }
    ...
    window.projetosChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                ...
                },
                {
                    ...
                    pointHoverRadius: 8,
                    pointStyle: 'circle',
                    order: 1, // Linhas ficam na frente
                    datalabels: {
                        anchor: 'end',
                        align: 'bottom',
                        offset: 35,
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Quantidade de Projetos e seus %',
                    font: {
                        size: 18,
                        weight: 'bold'
                    },
            ...
            scales: {
                x: {
                    ...
                    ticks: {
                        ...
                        callback: function(value, index, values) {
                            const label = this.getLabelForValue(value);
                            // Mostrar nomes completos para cidades
                            return label;
                        }
                    },
                    grid: {
...
function updateSupervisorStatusChartWithData(data) {
    console.log('🔄 Atualizando gráfico supervisor/status com dados filtrados...');
    ...
    if (data && Array.isArray(data)) {
        // Obter nomes exatos das colunas da tabela dinâmica
        const columnNames = getDynamicTableColumnNames();
        const supervisorCol = columnNames.SUPERVISOR;
        const statusCol = columnNames.STATUS;
        
        console.log('🔍 Colunas para supervisor/status:', { supervisorCol, statusCol });
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: function(context) {
                            ...
                            return `${context.dataset.label}: ${value} (${percentual}%)`;
                        },
                        footer: function(tooltipItems) {
                            const total = tooltipItems.reduce((sum, item) => sum + item.parsed.y, 0);
                            return `Total: ${total}`;
    ...
    console.log('✅ Gráfico supervisor/status atualizado com dados filtrados');
}
...
function generateEquipeRanking() {
    ...
    // Se não há dados suficientes, retornar dados vazios
...
// Função de atualização removida - não mais necessária
...
function exportEquipeRanking() {
    ...
    const worksheetData = [
        ['Ranking', 'Equipe', 'ATIVAÇÃO', 'CONSTRUÇÃO', 'VISTORIA', 'Total Geral']
    ];
    ...
    worksheetData.push([
        '',
        'Total Geral',
        rankingData.totais.ativacao,
        rankingData.totais.construcao,
        rankingData.totais.vistoria,
        rankingData.totais.geral
    ]);
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Ranking Equipes');
    
    // Download do arquivo
    const fileName = `ranking_equipes_${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(workbook, fileName);
    
    showSuccess('Exportação Concluída!', 'O ranking foi exportado em formato XLSX!');
}

// Função para atualizar ranking das equipes com dados filtrados
function updateEquipeRankingWithData(data) {
    console.log('🔄 Atualizando ranking das equipes com dados filtrados...');
    ...
    if (data && Array.isArray(data)) {
        // Obter nomes exatos das colunas da tabela dinâmica
        const columnNames = getDynamicTableColumnNames();
        const equipeCol = columnNames.EQUIPE;
        const tipoAcaoCol = columnNames['TIPO DE AÇÃO'];
        
        console.log('🔍 Colunas para ranking:', { equipeCol, tipoAcaoCol });
        ...
        if (equipeCol && tipoAcaoCol) {
            data.forEach(row => {
                ...
                if (equipe && equipe !== '' && tipoAcao && tipoAcao !== '') {
                    if (!equipeData[equipe]) {
                        equipeData[equipe] = { ATIVACAO: 0, CONSTRUCAO: 0, VISTORIA: 0, total: 0 };
                    }
                    
                    const tipoNormalizado = tipoAcao.toUpperCase();
                    if (tipoNormalizado.includes('ATIVAÇÃO') || tipoNormalizado.includes('ATIVACAO')) {
                        equipeData[equipe].ATIVACAO++;
                        totalAtivacao++;
                    } else if (tipoNormalizado.includes('CONSTRUÇÃO') || tipoNormalizado.includes('CONSTRUCAO')) {
                        equipeData[equipe].CONSTRUCAO++;
                        totalConstrucao++;
                    } else if (tipoNormalizado.includes('VISTORIA')) {
                        equipeData[equipe].VISTORIA++;
                        totalVistoria++;
                    }
    ...
    console.log('✅ Ranking das equipes atualizado com dados filtrados');
}

// Função para atualizar ranking das equipes por status com dados filtrados
function updateEquipeStatusRankingWithData(data) {
    console.log('🔄 Atualizando ranking das equipes por status com dados filtrados...');
    ...
    if (data && Array.isArray(data)) {
        ...
        console.log('🔍 Colunas para ranking de status:', { equipeCol, statusCol });
            ...
            data.forEach(row => {
                ...
                if (equipe && equipe !== '' && status && status !== '') {
                    if (!equipeStatusData[equipe]) {
                        equipeStatusData[equipe] = { PRODUTIVA: 0, IMPRODUTIVA: 0, total: 0 };
                    }
    ...
    const equipesOrdenadas = Object.entries(equipeStatusData)
        ...
        .map(([equipe, dados], index) => ({
            ...
            equipe,
            ...dados,
            produtividade: dados.total > 0 ? ((dados.PRODUTIVA / dados.total) * 100).toFixed(1) : 0
        }));
    ...
    if (tableBody) {
        ...
        equipesOrdenadas.forEach((equipe, index) => {
            ...
            const produtividadeClass = getProdutividadeClass(equipe.produtividade);
            
            row.innerHTML = `
                <td>${equipe.ranking}</td>
                <td>${equipe.equipe}</td>
                <td>${equipe.PRODUTIVA.toLocaleString()}</td>
                <td>${equipe.IMPRODUTIVA.toLocaleString()}</td>
                <td><strong>${equipe.total.toLocaleString()}</strong></td>
                <td><span class="produtividade-badge ${produtividadeClass}">${equipe.produtividade}%</span></td>
            `;
        ...
        const totalGeralEl = document.getElementById('totalGeralStatus');
        ...
        const produtividadeGeral = totalGeral > 0 ? ((totalProdutiva / totalGeral) * 100).toFixed(1) : 0;
        if (totalProdutividadeEl) {
            const produtividadeClass = getProdutividadeClass(produtividadeGeral);
            totalProdutividadeEl.innerHTML = `<span class="produtividade-badge ${produtividadeClass}">${produtividadeGeral}%</span>`;
        }
    }

    console.log('✅ Ranking das equipes por status atualizado com dados filtrados');
}
...
function exportEquipeStatusRanking() {
    ...
    // Preparar dados para XLSX
    const worksheetData = [
        ['Ranking', 'Equipe', 'PRODUTIVA', 'IMPRODUTIVA', 'Total', 'Produtividade (%)']
    ];
    ...
    worksheetData.push([
        '',
        'Total Geral',
        rankingData.totais.produtiva,
        rankingData.totais.improdutiva,
        rankingData.totais.geral,
        rankingData.totais.percentual
    ]);
    ...
    // Adicionar worksheet ao workbook
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Ranking Status');
    
    // Download do arquivo
    const fileName = `ranking_equipes_status_${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(workbook, fileName);
    
    showSuccess('Exportação Concluída!', 'O ranking por status foi exportado em formato XLSX!');
}

// Exportar todos os rankings em um único arquivo XLSX
function exportAllRankings() {
    console.log('📤 Exportando todos os rankings...');
    
    const rankingData = generateEquipeRanking();
    const statusRankingData = generateEquipeStatusRanking();
    
    // Criar workbook
    const workbook = XLSX.utils.book_new();
    
    // Worksheet 1: Ranking por Tipo de Ação
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet1, 'Por Tipo de Ação');
    
    // Worksheet 2: Ranking por Status
    const worksheet2Data = [
        ['Ranking', 'Equipe', 'PRODUTIVA', 'IMPRODUTIVA', 'Total', 'Produtividade (%)']
    ];
    
    statusRankingData.equipes.forEach(equipe => {
        worksheet2Data.push([
            equipe.ranking,
            equipe.equipe,
            equipe.PRODUTIVA,
    ...
    worksheet2Data.push([
        '',
        'Total Geral',
        statusRankingData.totais.produtiva,
        statusRankingData.totais.improdutiva,
        statusRankingData.totais.geral,
        statusRankingData.totais.percentual
    ]);
    
    const worksheet2 = XLSX.utils.aoa_to_sheet(worksheet2Data);
    XLSX.utils.book_append_sheet(workbook, worksheet2, 'Por Status');
    
    // Download do arquivo
    const fileName = `rankings_completos_${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(workbook, fileName);
    
    showSuccess('Exportação Concluída!', 'Todos os rankings foram exportados em formato XLSX!');
}
...
window.debugDadosPerdidos = debugDadosPerdidos;

// Função para debug dos filtros
function debugFiltros() {
    console.log('🔍 DEBUG: Verificando filtros...');
    
    // Verificar se os elementos dos filtros existem
    const filtros = {
        projeto: document.getElementById('infraFilterProjeto'),
        subProjeto: document.getElementById('infraFilterSubProjeto'),
        equipe: document.getElementById('infraFilterEquipe'),
        status: document.getElementById('infraFilterStatus'),
        cidade: document.getElementById('infraFilterCidade'),
        supervisor: document.getElementById('infraFilterSupervisor'),
        tipoAcao: document.getElementById('infraFilterTipoAcao')
    };
    
    console.log('📋 Elementos dos filtros:', filtros);
    
    // Verificar valores atuais dos filtros
    const valores = {};
    Object.entries(filtros).forEach(([nome, elemento]) => {
        valores[nome] = elemento ? elemento.value : 'Elemento não encontrado';
    });
    
    console.log('🔍 Valores atuais dos filtros:', valores);
    
    // Verificar dados disponíveis
    console.log('📊 Dados disponíveis:');
    console.log('  - dynamicTableData:', dynamicTableData ? 'Disponível' : 'Não disponível');
    console.log('  - Headers:', dynamicTableData?.headers || 'Não disponível');
    console.log('  - Total de registros:', dynamicTableData?.data?.length || 0);
    ...
    // Testar filtragem
    if (dynamicTableData?.data?.length > 0) {
        const projeto = filtros.projeto?.value;
        if (projeto && projeto !== '') {
            console.log(`🧪 Testando filtro para projeto: "${projeto}"`);
            
            const filtered = dynamicTableData.data.filter(row => {
                return columnNames.PROJETO && row[columnNames.PROJETO]?.toString().trim() === projeto;
            });
            
            console.log(`📊 Resultado do teste: ${filtered.length} registros encontrados`);
            console.log('🔍 Exemplo de dados filtrados:', filtered.slice(0, 3));
        }
    }
}

// Função para debug dos gráficos
function debugGraficos() {
    console.log('📊 DEBUG: Verificando gráficos...');
    
    const graficos = {
        projetos: window.projetosChart,
        subProjetos: window.subProjetosChart,
        cidades: window.cidadesChart,
        hpProjetos: window.hpProjetosChart,
        supervisorStatus: window.supervisorStatusChart
    };
    
    console.log('📋 Status dos gráficos:', graficos);
    
    // Verificar se os elementos canvas existem
    const canvas = {
        projetos: document.getElementById('projetosChart'),
        subProjetos: document.getElementById('subProjetosChart'),
        cidades: document.getElementById('cidadesChart'),
        hpProjetos: document.getElementById('hpProjetosChart'),
        supervisorStatus: document.getElementById('supervisorStatusChart')
    };
    
    console.log('📋 Elementos canvas:', canvas);
    
    // Testar recriação de um gráfico
    if (canvas.subProjetos) {
        console.log('🧪 Testando recriação do gráfico de sub projetos...');
        try {
            updateSubProjetosChartWithData(dynamicTableData?.data || []);
            console.log('✅ Gráfico de sub projetos recriado com sucesso');
        } catch (error) {
            console.error('❌ Erro ao recriar gráfico de sub projetos:', error);
        }
    }
...
// Função para debug específico do problema de dados perdidos
function debugDadosPerdidos() {
    console.log('🔍 DEBUG: Verificando problema de dados perdidos...');
    
    // Verificar dados originais
    console.log('📊 Dados originais:', {
        total: dynamicTableData?.data?.length || 0,
        headers: dynamicTableData?.headers || [],
        sample: dynamicTableData?.data?.slice(0, 3) || []
    });
    ...
    if (dynamicTableData?.data?.length > 0 && columnNames.STATUS) {
        // Contar status diferentes
        const statusCount = {};
        const supervisorStatusCount = {};
        
        dynamicTableData.data.forEach(row => {
            const status = row[columnNames.STATUS]?.toString().trim().toUpperCase();
            const supervisor = row[columnNames.SUPERVISOR]?.toString().trim();
            ...
            if (supervisor && status) {
                if (!supervisorStatusCount[supervisor]) {
                    supervisorStatusCount[supervisor] = { PRODUTIVA: 0, IMPRODUTIVA: 0 };
                }
                
                if (status === 'PRODUTIVA') {
                    supervisorStatusCount[supervisor].PRODUTIVA++;
                } else if (status === 'IMPRODUTIVA') {
                    supervisorStatusCount[supervisor].IMPRODUTIVA++;
                }
            }
        });
        
        console.log('📊 Contagem de status:', statusCount);
        console.log('📊 Status por supervisor:', supervisorStatusCount);
        
        // Testar função de atualização
        console.log('🧪 Testando função updateSupervisorStatusChartWithData...');
        try {
            updateSupervisorStatusChartWithData(dynamicTableData.data);
...
// Função para atualizar horário da última atualização da infraestrutura
function updateInfraLastUpdateTime() {
    const now = new Date();
    ...
    const timeElement = document.getElementById('infraLastUpdateTime');
    if (timeElement) {
        timeElement.textContent = `Atualizado em ${formattedTime}`;
    }
    
    console.log('🕐 Horário da última atualização atualizado:', formattedTime);
}
